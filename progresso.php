...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    elseif ($acao === 'criar_checklist') {
        ...
        if ($id_habito > 0) {
            // Verifica se já existe checklist para este hábito nesta data
    ...
    elseif ($acao === 'deletar_habito') {
        ...
        // Depois remove o hábito
        $delete = "DELETE FROM habito WHERE id_habito = ? AND id_user = ?";
        $stmt = $conn->prepare($delete);
        $stmt->bind_param("ii", $habito_id, $user_id);
        if ($stmt->execute()) {
            $mensagem = "✅ Hábito removido com sucesso!";
        } else {
            $mensagem = "❌ Erro ao remover hábito.";
        }
        $stmt->close();
    }
...
// Buscar checklists diários de hoje (com JOIN para pegar dados do hábito)
$checklists = [];
...
<body>
        ...
        <div class="progresso-container">
                ...
                <div class="categoria-content">
                    ...
                    <div class="card-progresso">
                        ...
                        <div class="progresso-bar">
                            <div class="progresso" style="width: <?php echo min(100, ($agua_meta > 0 ? ($agua_hoje / $agua_meta) * 100 : 0)); ?>%;"></div>
                        </div>
                        <div class="meta-header">
                            <div class="progresso-percentual">
                                Meta: <?php echo $agua_meta; ?>L (<?php echo number_format(($agua_meta > 0 ? ($agua_hoje / $agua_meta) * 100 : 0), 0); ?>%)
                            </div>
                    ...
                    <div class="card-progresso">
                        ...
                        <form method="POST" class="form-progresso">
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" id="quantidade_agua" name="quantidade" step="0.1" min="0" max="5" value="0.5" required>
                            </div>
                            <div class="form-group">
                                <label for="data_agua">Data</label>
                                <input type="date" id="data_agua" name="data" value="<?php echo date('Y-m-d'); ?>" required>
                            </div>
                    ...
                    <div class="card-progresso">
                        ...
                        <form method="POST" class="form-progresso">
                            ...
                            <div class="form-group">
                                ...
                                <input type="date" id="data_peso" name="data" value="<?php echo date('Y-m-d'); ?>" required>
                            </div>
                            <button type="submit" class="btn-adicionar">
                    ...
                    <div class="card-progresso">
                        ...
                        <form method="POST" class="form-progresso">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" id="descricao" name="descricao" placeholder="Ex: Arroz, frango, salada">
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <input type="date" id="data_alimentacao" name="data" value="<?php echo date('Y-m-d'); ?>" required>
                            </div>
                            <button type="submit" class="btn-adicionar">
                                <i class="fas fa-plus"></i> Adicionar
                            </button>
            ...
            <div class="categoria-item fade-in-element">
                ...
                <div class="card-progresso" style="margin-bottom: 25px;">
                    <h4>Criar Novo Hábito</h4>
                    <form method="POST" class="form-progresso">
                        <input type="hidden" name="acao" value="criar_habito">
                        <div class="form-group">
                            <label for="descricao_habito">Descrição do Hábito</label>
                            <input type="text" id="descricao_habito" name="descricao" placeholder="Ex: Beber 3L de água" required>
                        </div>
                        <div class="form-group">
                            <label for="tipo_habito">Tipo (opcional)</label>
                            <input type="text" id="tipo_habito" name="tipo" placeholder="Ex: Saúde, Exercício, Alimentação">
                        </div>
                        <div class="form-group">
                            <label for="meta_diaria">Meta Diária (opcional)</label>
                            <input type="number" id="meta_diaria" name="meta_diaria" step="0.1" min="0" placeholder="Ex: 3.0">
                        </div>
                        <button type="submit" class="btn-adicionar">
                ...
                <!-- Lista de hábitos/desafios permanentes -->
                <?php if (!empty($habitos)): ?>
                <div style="margin-bottom: 25px;">
                    <h4 style="margin-bottom: 15px;">Meus Desafios</h4>
                    <?php foreach ($habitos as $habito): ?>
                        <div class="checklist-item" style="background: var(--cor-primaria); border: 1px solid var(--cor-secundaria);">
                            ...
                            <div class="checklist-actions">
                                <form method="POST" style="display: inline;" onsubmit="return confirm('Tem certeza que deseja remover este desafio?');">
                ...
                <?php else: ?>
                    <p style="text-align: center; color: var(--cor-texto); opacity: 0.7; padding: 20px;">
                        Nenhum desafio criado ainda. Crie um hábito acima ou complete o onboarding para receber desafios automáticos!
                    </p>
                <?php endif; ?>
        ...
        <div class="modal-content" onclick="event.stopPropagation()">
            ...
            <form method="POST" class="form-progresso">
                ...
                <div class="form-group">
                    ...
                    <input type="number" id="meta_agua" name="meta_agua" step="0.1" min="0" max="10" value="<?php echo $agua_meta; ?>" required>
                </div>
                <button type="submit" class="btn-adicionar">
    ...
    <nav class="navbar">
        ...
        <a href="progresso.php" class="nav-link active"><i class="fas fa-chart-line icon"></i> <span class="text">Progresso</span></a>
